...
function RequirementBlock({name, depth, subReqs, subCourses, checkbox}: {name:string, depth: number, subReqs: Requirement[], subCourses: Course[], checkbox: boolean}) {
    ...
    <>
      <div className={border_style}>
        <Disclosure>
          { checkbox ? (
            <>
              {/* Make a checkbox appear in the requirement block */}
            </>
          ) : (
            <div className="grid grid-cols-8 items-center">
              ...
              <p className="col-span-4 line-clamp-1 justify-self-start">{name}</p>
              <div className="col-span-3 justify-self-end w-full max-w-[140px] mr-[15px]"><ProgressBar></ProgressBar></div>
            </div>
          )}
          { subReqs.length == 0 ? (
            ...
            <DisclosurePanel className="flex flex-col gap-[12px] col-span-6">
              {subReqs.map((req) =>
                <RequirementBlock key={req.id} name={(depth <= 1 ? "" : req.id + ". ") + req.name} depth={depth + 1} subReqs={req.subReqs} subCourses={req.subCourses} checkbox={false}></RequirementBlock>
              )}
            </DisclosurePanel>
          )}
        </Disclosure>
