		...
		<>
			<div className="border-3 rounded-lg w-full h-[140px] p-1 flex flex-col">
				...
				{tags.includes("T") ? (
					...
					<>
						<p className="text-md text-center self-center line-clamp-2 max-w-8/10">{course.TransferCredit?.TransferCourseEquivalency?.TransferSchool?.schoolName as string}</p>
						<p className="text-md text-center self-center line-clamp-2 max-w-8/10">{course.Course.name}</p>
					</>
				)}
